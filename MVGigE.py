...
class MVImageCreate():
    ...
    def __init__(self, width, height, nBPP):
        ...
        self.nBPP = c_uint64(nBPP)
        self.init()

    def init(self):
        MVGigE.MVImageCreate.restype = POINTER(c_ulonglong)
...
class MVSetExposureAuto():
    ...
    status：状态码
    """
    def __init__(self, hCam, pExposureAuto):
        self.hCam = c_uint64(hCam)
        self.pExposureAuto = c_int(pExposureAuto.value)
        self.init()

    def init(self):
        result = MVGigE.MVSetExposureAuto(self.hCam, self.pExposureAuto)
        self.status = MVSTATUS_CODES(result)


class MVGetExposureAuto():
    """
    读取自动曝光模式
    输入：相机句柄
    返回两个属性
    ...
    def init(self):
        pExposureAuto = c_int()
        result = MVGigE.MVGetExposureAuto(self.hCam, byref(pExposureAuto))
        self.status = MVSTATUS_CODES(result)
        self.pExposureAuto = ExposureAutoEnums(pExposureAuto.value)


class MVSetBalanceWhiteAuto():
    """"
    设置自动白平衡模式
    ...
    status：状态码
    """
    def __init__(self, hCam, pBalanceWhiteAuto):
        self.hCam = c_uint64(hCam)
        self.pBalanceWhiteAuto = c_int(pBalanceWhiteAuto.value)
        self.init()

    def init(self):
        result = MVGigE.MVSetBalanceWhiteAuto(self.hCam, self.pBalanceWhiteAuto)
        self.status = MVSTATUS_CODES(result)


class MVGetBalanceWhiteAuto():
    """
    读取自动白平衡模式
    输入：相机句柄
    返回两个属性
    ...
    def init(self):
        pBalanceWhiteAuto = c_int()
        result = MVGigE.MVGetBalanceWhiteAuto(self.hCam, byref(pBalanceWhiteAuto))
        self.status = MVSTATUS_CODES(result)
        self.pBalanceWhiteAuto = BalanceWhiteAutoEnums(pBalanceWhiteAuto.value)
...
class MVSetGainAuto():
    ...
    status：状态码
    """
    def __init__(self, hCam, pGainAuto):
        self.hCam = c_uint64(hCam)
        self.pGainAuto = c_int(pGainAuto.value)
        self.init()

    def init(self):
        result = MVGigE.MVSetGainAuto(self.hCam, self.pGainAuto)
        self.status = MVSTATUS_CODES(result)
...
class MVGetGainAuto():
    ...
    def init(self):
        pGainAuto = c_int()
        result = MVGigE.MVGetGainAuto(self.hCam, byref(pGainAuto))
        self.status = MVSTATUS_CODES(result)
        self.pGainAuto = GainAutoEnums(pGainAuto.value)