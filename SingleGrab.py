...
class MVCam(QWidget):
    ...
    def initUI(self):
        ...
        # 加入表格
        self.table = QTableWidget(27, 4)  # 添加右侧表格
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)  # 设置表格为自适应的伸缩模式，即可根据窗口的大小来改变网格的大小
        # self.table.setEditTriggers(QAbstractItemView.NoEditTriggers)  # 设置为只读表格
...
class CreateSampleUI(QWidget):
    ...
    def initUI(self):
        ...
        self.Samplelabel = MyLabel(self)
        self.ScrollArea = QScrollArea(self)
        self.ScrollArea.setBackgroundRole(QPalette.Dark)
        self.ScrollArea.setWidget(self.Samplelabel)

        self.Sample_winid = self.Samplelabel.winId()  # 获取label对象的句柄
        self.Samplelabel.setStyleSheet("QLabel{background:Dark;}")
        QHbox = QHBoxLayout()
        QHbox.addWidget(self.btnSaveSampleImg)
        QHbox.addWidget(self.btnCreateSample)
        QHbox.addStretch(1)
        Qvbox = QVBoxLayout()
        Qvbox.addLayout(QHbox)
        Qvbox.addWidget(self.ScrollArea)
        self.setLayout(Qvbox)

        MVStartGrabWindow(self.hCam, self.Sample_winid)  # 将采集的图像传输到指定窗口

        w = MVGetWidth(self.hCam)  # 获取图像宽度
        h = MVGetHeight(self.hCam)  # 获取图像高度
        width = w.width
        height = h.height
        pf = MVGetPixelFormat(self.hCam)  # 获取图像格式
        pixelFormat = pf.pixelFormat
        if (pixelFormat == MV_PixelFormatEnums.PixelFormat_Mono8):
            self.Sampleimage = MVImageCreate(width, height, 8).himage  # 创建图像句柄
        else:
            self.Sampleimage = MVImageCreate(width, height, 24).himage  # 创建图像句柄

        self.Samplelabel.resize(width, height)

        #     绑定点击事件
        self.btnSaveSampleImg.clicked.connect(self.saveSampleImg)
    ...
    def saveSampleImg(self):  # 保存图片执行本函数，在非触发模式时，只有采集暂停是才可以保存
        ...
        if ok:
            try:
                ...
                newfile = '\\'.join(fname.split('/')[:-1])  # 需要存储到的新文件夹
                print(pathname)
                MVImageSave(self.Sampleimage, filename.encode('utf-8'))  # 将图片保存下来
                if (newfile != os.getcwd()):  # 将图片移动到指定文件夹下
                    try:
                        shutil.move(pathname, newfile)
                    except:
    ...
    def createSample(self):
        MVFreezeGrabWindow(self.hCam, True)
        fname, ok = QFileDialog.getOpenFileName(self, '选择图片', '.bmp', "Images (*.bmp *.jpg *.tif *.raw)")
        if ok:
            self.Samplelabel.rectS = {}  # 清空上一次的roi
        image = QImage(fname)
        h = image.width()
        w = image.height()
        print(w, h)
        self.Samplelabel.resize(h, w)
        self.Samplelabel.setPixmap(QPixmap.fromImage(image))  # 加载图片
        self.Samplelabel.setCursor(Qt.CrossCursor)
